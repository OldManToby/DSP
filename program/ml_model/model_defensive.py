...
class DefensiveModel:
    ...
    def load_data(self):
        # Load data from defensive subfolders
        self.selected_teams_data = self.load_csv_defensive()
        if self.selected_teams_data is not None and not self.selected_teams_data.empty:
            print(f"DefensiveModel: Data loaded successfully for {self.away_team} and {self.home_team}")
        else:
            print(f"DefensiveModel: No data available for selected teams: {self.away_team} and {self.home_team}")
    ...
    def display_summary(self):
        # Display defensive model summary or any other necessary information
        pass
